...
The work laid out in this thesis set out to answer the question of _How can an open-source microvasculature extraction pipeline be developed, able to be used by non-computer scientists, leveraging classical segmentation methods and sparse user-driven input to produce improved segmentations when compared to thresholding on CECT data across a diverse dataset?_, which was answered in the form of CollaboratiVessel, the 3D Slicer plugin that combines vesselness and intensity priors with vessel reconstruction steps leveraging user placed points with minimal hyperparameter tuning.
#linebreak()
In doing this work, the principal contributions are:

1. *A user friendly 3D Slicer plugin.* CollaboratiVessel is packaged as a plugin for 3D Slicer, requires no coding to operate, and can export binary segmentations compatible with downstream analysis tools. The UI was designed with prospective users in mind, offering only the necessary options, and the code was written in a modular fashion to enable simple changes and future expansion.

2. *A user-in-the-loop segmentation approach.* As opposed to classical deep learning approaches, where the algorithm, user and data are all treated separately and the algorithm is run without user interation, the pipeline in this work  starts from user inputs based on what the user immediately sees, derives its segmentation from user-placed annotations, and directly shows the user the results in a 3DSlicer segmentation, enabling rapid iteration and adjustment by modifying hyperparameters or revising segmentations with built-in 3D Slicer tools if required.

3. *A manually annotated evaluation dataset.* Six 3D subvolumes spanning four tumour samples were annotated, a valuable resource for future work on CECT microvasculature segmentation.

4. *A framework for extrapolative prediction* As opposed to purely hard evidence based methods like thresholding that do not extrapolate or interpolate vessels, or purely data driven methods for extrapolation and interpolation that require learning from data, this thesis shows that a multistep approach enforcing certain priors and based on an evidence accumulation framework can successfully extract connected vessel segmentations.
