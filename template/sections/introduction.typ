...
When carrying out research, the process of data acquisition, processing and analysis requires expertise across a vast amount of diverse domains. It is natural for researchers to not have extensive knowledge across all fields required for a particular experiment, which is compensated for by relying on the existing corpus of prior research work. However replicability is a major challenge across all disciplines: prior research work often fails to be reproducible or extensible due to a lack of accessibility to the data acquisition hardware and methods, data processing tools, insufficiently documented procedures, or subjective biases resulting from prior experience of the person executing the process @10simple_rules_reproducibility. Free and open source tools paliate the data processing replicability issue, publication of the data as well as data versionning enable an understanding of the steps carried out modifying the data, and automation in processes such as segmentation remove the tacit data filtering done by a human in the annotation process.
...
#linebreak()
Histology is relevant in both clinical and laboratory environments. Clinically, histopathological analysis of tissue samples is extensively used for use cases such as cancer detection and grading @histology_used_for_cancers_he2012histology where a sample is analysed manually or with computer assistance. In laboratories for tasks such as clinical trials, histopathologists carry out manual microscopic evaluation @pathology_in_clinical_trials_provenzano2015standardization. Classical 2D histology presents the distinct disadvantage of being unable to obtain information at the same granularity in all three axis: slices may be stacked to obtain a 3D result, but resolution in the third axis is limited to section thickness, with the stacked planes separated by a dead space where no information is acquired @litt_review_greet_debournonville2019contrast, (adding a figure here for the loss in resolution is difficult to find?) and the cutting process induces changes in tissue structure @extending2d_histo_to_3d. 

#linebreak()
All types of tissues can be analysed histologically. For the purpose of evaluating the use of the antiangiogenic drug Pazopanib, a tyrosine kinase inhibitor which blocks the creation of new blood vessels, as a means for cancer treatment, accurate 3d reconstruction of blood vessel vascularization is required due to simple volume estimates not capturing important parameters such as tortuosity, connectivity/branching and cross section profile. The shortcomings of classical 2D histology make the asessment and quantification of hollow and highly heterogeneous tissues like vasculature, for which spatial relationships are particularly relevant, challenging @litt_review_greet_debournonville2019contrast. Due to these limitations alternatives such as 3D imaging methods that do not require slicing and have high spatial resolution in all axis are particularly relevant for blood vessel analysis.

#linebreak()
To collect 3D data, the simplest is to extend 2D by stacking slices @methods_for_3d_histo_pichat2018survey. This method limits the resolution across the slicing axis, and does not alleviate the tissue deformation challenges. Non invasive 3D imaging methods rely on wavelengths able to further penetrate the tissue to be imaged: visible light can be used for shallow 3D imaging of soft tissues, with longer wavelengths such as x-rays able to penetrate completely through soft tissue as well as cartilage and bone. Although the machines and methods exist to collect high resolution 3D data using various wavelengths and associated techniques, such as µMRI and µCT, the analysis of high resolution 3D data is not as straightforward as with 2D histological slices: there are many more slices, often thousands, requiring software to assemble these into a 3D volume, and exponentially more time for manual annotation. Methods for segmenting in 3D lack standardization: there is high scan to scan variance between different tissue types, wide array of machines and machine settings, different contrast agents used at different concentrations and with different methodologies.

#linebreak()
Our method of choice for analyzing blood vessels, µCT, presents low contrast between soft tissue: techniques exist to increase contrast and improve visibility such as cryo-CT @cryoct_maes2022cryogenic or the utilization of contrast agents in contrast enhanced CT, called CECT. Although essential to increase the contrast between tissue types, these techniques still do not enable sufficient differentiation for small vascular networks such as those found in the murine tumors collected for the evaluation of Pazopanib to allow threshold simple based segmentation, as is commonly used for bone or samples with high contrast agent presence @litt_review_greet_debournonville2019contrast. (should I link here to Lisa's thesis?)

#linebreak()
Alternatives to threshold based segmentation exist, however due to subjectivity and annotation techniques, these suffer from strong variance between annotators @variability_in_annotations_xray_lin2023pluribus. This variance comes from multiple sources: expertise difference, tool type and experience, available time, subjectivity and more. This variance, combined with the time required to manually segment, means that small vascular networks remain difficult to obtain complete segmentations for, reducing the replicability of research and introducing noise into the data, increasing uncertainty margins.

#linebreak()
Finally, when a segmentation can be obtained, it is often plagued by issues stemming from the nature of prior free segmentation techniques like threslhold or non context aware methods: discontinuities in blood vessels, stemming from the lack of explicit vesselness constraints such as spatial continuity, mean that the final segmentation lacks the ability to be evaluated based on higher level parameters like tortuosity, and instead are purely evaluated on volume (here again lisa? or her reference?).

#linebreak()
In this thesis work will be carried out to create an open source, user friendly and robust small blood vessel extraction method for obtaining clinically relevant vasculature characteristics from µCT data acquired in the context of the testing of Pazopanib, with a focus on ensuring re-usability and adoption by researchers. The pipeline will use 3DSlicer as a basis and take in and output data in portable data formats, to ensure re-usability of the work. Proven algorithms with user adjustable hyperparameters to enable easy re-use without requiring expertise in computer science are to be used. Implemented methods will be compared with existing pipelines for extracting fine blood vessels, and tested for robustness by examining performance on data previously considered non segmentable due to non uniform contrast agent staining. 
