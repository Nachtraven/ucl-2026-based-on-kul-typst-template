...
#linebreak()
Alternatives to threshold based segmentation exist, however due to subjectivity and annotation techniques, these suffer from strong variance between annotators @variability_in_annotations_xray_lin2023pluribus. This variance comes from multiple sources: expertise difference, tool type and experience, available time, subjectivity and more [source this based on above two]. This high variance, combined with the time required to manually segment, means that small vascular networks remain difficult to obtain complete segmentations for.
...
#linebreak()
In this thesis work will be carried out to create an open source, user friendly and robust extraction method for obtaining clinically relevant vasculature characteristics from µCT data acquired in the context of the testing of Pazopanib, with a focus on ensuring re-usability and adoption by researchers. The pipeline will take in and output data in portable data formats, and use proven algorithms with user adjustable hyperparameters to enable easy re-use without requiring expertise in computer science. Implemented methods will be compared with existing pipelines for extracting fine blood vessels, and tested for robustness by examining performance on data previously considered non segmentable due to non uniform contrast agent staining. 
