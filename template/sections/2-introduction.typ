...
Biological tissue function depends strongly on spatial organization of its constituents, its cells and extracellular matrix. This is particularly true for vascular networks in organs and tumors, whose branching geometry, vessel diameters, and connectivity together determine how functional the vascularization is. Interpretation of the effect of diseases or drug treatments on tissue vascularization depends therefore on accurate imaging data extraction and subsequent structural quantification

These structures have higher-order structural properties: vessel density, diameter distributions, branching frequency and changes in tortuosity which cannot be reliably recovered from 2D images, nor from voxel-level metrics in 3D datasets that ignore network topology. This 3D nature motivates the use of high-resolution 3D imaging able to discern small vessels inside of tissues. 

In this work, CECT imaging is used on ex-vivo [Briefly introduce CECT]

The datasets used represent murine tumors and their vascularization @fig:cect-data-examples

// For the dataset used in this work, contrast-enhanced Micro-CT scans were acquired using an agent that increases the attenuation of small blood vessels, with specific properties laid out in @fig:cect-data-examples.

#linebreak()
Existing tools for analyzing CECT data are dominated by proprietary paid software, and the available open-source alternatives lack robust solutions for extracting small-scale vasculature with its challenging specificities: vessels only few voxels across, with low contrast and disconnections, background tissue with high signal non-vessel-like structures and contrast gradients. As a result, standard segmentation approaches failed to accurately extract the vessels. //the vasculature in the provided dataset has resisted prior analysis attempts.
...
#linebreak()
Analyzing tissue requires imaging it at a sufficient resolution to extract the structure of interest, and 3D imaging modalities leveraging tomography such as CECT now make this possible at micrometer resolution. However 3D imaging carries with it challenges: a single high-resolution CECT dataset can reach into the thousands of slices, with large scans on the order of 3000³ at 16 bits per voxel. This results in orders of magnitude more data than a comparable 2D acquisition, meaning that manual analysis as with 2D microscopy is impractical. CECT data itself is also highly variable: datasets differ on the same machine due to changes in imaged tissue, contrast agents and protocols used to make structures of interest visible, and further variation arises across machines and due to researchers' choices of acquisition parameters. 
