...
However, existing tools tools for analyzing Micro-CT data are dominated by proprietary paid software and the available open-source alternatives lack robust solutions to extract small-scale vasculature with its challenging specificities: low contrast, non vessel-lieke structures, diffusion gradients, and vessels only a few voxels across. As a result, the vasculature in the provided datasets has resisted prior analysis attempts.
