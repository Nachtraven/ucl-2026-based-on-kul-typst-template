...
//Research takes place at the edges of human knowledge, and extends it through careful, iterative work. 
In the field of biology, the cutting edge in analysis of tissues has evolved from 2D histology, carried out by humans looking through microscopes to a wide variety of 3D techniques across different wavelenghts ranging from sound and infrared to magnetic resonance and x-ray imaging. This evolution has permitted ever higher fidelity reconstruction of tissues, driven by the importance of tissue structure in its function. The progress from 2D to 3D has increased data generation by orders of magnitude, bringing with it the need for more advanced tools and methods to help in analyzing the data. These tools and associated knowledge has grown across different fields in parallel as different requirements and use cases drove development. Tissue analysis has naturally also seen the uptake of software tools, however the creation and extension of these tools, and the algorithms that drive image processing more generally, fall outside of the domain of expertise of most Biologists, this results in a reliance on prior work and existing tools and pipelines. This prior work is often not directly applicable or re-usable   When carrying out research it is common to utilize processes and methods that extend beyond the boundaries of said field. In biology and materials science, the process of data acquisition, processing and analysis requires expertise across a vast amount of different domains: physics to understand the imaging methods used, chemistry relating to the reactants employed for colour and constrast, and computer science in the methods used for analysis and information extraction from collected data. It is natural for researchers to not have extensive knowledge across all fields required for a particular experiment, which is compensated for by relying on the existing corpus of prior research work, established methods, and available tools.
...
Microscopy as a technique and method for tissue analysis predates the field of computer science. As mentioned, 2D microscopy has downsides and the field has progressed towards more and more data intensive and computerized imaging techniques, namely 3D methods. To collect 3D data, the simplest is to extend 2D techniques by stacking slices @methods_for_3d_histo_pichat2018survey. This method limits the resolution across the slicing axis, and does not alleviate the tissue deformation challenges, although is doable without extensive computerization. Non invasive 3D imaging methods rely on wavelengths able to further penetrate the tissue to be imaged: visible light can be used for shallow 3D imaging of soft tissues, with longer wavelengths such as x-rays able to penetrate completely through soft tissue as well as cartilage and bone, requiring a computerized reconstruction pipeline to generate final usable data. Although the machines and methods exist to collect high resolution 3D data using various wavelengths and associated techniques, such as µMRI and µCT, the analysis of high resolution 3D data is not as straightforward as with 2D histological slices: there are many more slices, often thousands, and would require exponentially more time for manual annotation. 

// Motivating the use of algorithms that have generalization capabilities, and motivating the need for unified software
#linebreak()
There is also the issue of data diversity stemming from the increase in dimensionality and available imaging methods: even for one given imaging "type" such as MRI, there is high scan to scan variance between different tissue types, a wide array of machines and machine settings, and different contrast or staining agents that can be used at different concentrations and with different methodologies for getting them into the tissue of interest. This huge diversity natually drives a fragmented and diverse software landscape with often duplicate work when easy to use or free and open tools are not available.

#linebreak()
Our imaging method of choice for analyzing blood vessels, µCT, presents low contrast between soft tissue: techniques exist to increase contrast and improve visibility such as cryo-CT @cryoct_maes2022cryogenic or the utilization of contrast enhacing staining agents (CESAs), as utilized for the data in this thesis, contrast-enhanced CT, called CECT. Although essential to increase the contrast between tissue types, these techniques still do not enable sufficient differentiation for small vascular networks such as those found in the murine tumors collected for the evaluation of Pazopanib to allow threshold based classification as is commonly used for bone or for samples with high contrast agent presence @litt_review_greet_debournonville2019contrast, this was shown during the analysis phase of @wlodarski, the source of the data used here, where threshold based segmentation was used and resulted in poor performance, motivating the improvements in this thesis.


== Computer science as a tool

#linebreak()
The two paths researchers may take when faced with more complex data are twofold: either manual annotation of the data, or utilization of algorithms to aid in the separation of their structures of interest. Manual annotation suffers from high subjectivity, strong variance between annotators @variability_in_annotations_xray_lin2023pluribus and requires time and expert annotators. The inter annotator variance can be explained in mutliple fashions: expertise difference, tool type and experience, available time, subjectivity and more. This variance, combined with the time required to manually segment, means that small vascular networks remain difficult to obtain complete segmentations for, reducing the replicability of research and introducing noise into the data, increasing uncertainty margins.
// Maybe here should mention that manual annotation or segmentation is still done for 2D slices and in hospitals?

// HERE IMPORTANT TO MENTION AUTOMATIC ML NN-UNET
#linebreak()
The variance in manual annotations, as well as time taken, motivate the need for automated or algorithm based annotation techniques, and drive users for whom the purpose of analyzing data is not to contribute or work on the process of analysis towards user friendly and simple methods where data analysis is a tool enabling downstream work. These algorithmic annotation techniques can themselves again be split into two categories: (1) generalist methods such as the aforementioned thresholding, simple to understand and widely available however having has poor performance on data that cannot easily be split purely based on grey values and (2) a wide spectrum of techniques that carry a richer prior or set of priors about the target structure. For our usecase of vessel analysis, simple methods are easy to apply but result in discontinuities in blood vessels and don't generalize across samples, as well as require the selection of a fixed threshold per sample, which as was discovered during user interviews is not done in a principled manner in practice. For more advanced techniques, these are often not explored due to the barrier to entry or friction associated with using a new tool or method, even though computer vision has progressed over the years towards including more and more expert knowledge, or priors, hand coded or saved in the form of weights in machine learning into the algorithms used for predictions.

//#linebreak()
//However, computer vision techniques focusing exclusively on the process of extraction of a segmentation on the pixel level mean that the final segmentation lacks the ability to be evaluated based on higher level parameters like tortuosity, and instead are purely evaluated on volume, surface area and other lower order parameters.

// Situation: computer science is not used outside of computer science labs and papers
#linebreak()
As mentioned previously, despite the availability of high performance algorithms for various tasks such as analysis of 2D microscopy slides, the uptake and adoption of these techniques remains limited outside of the walls of computer science labs: algorithms are often difficult to get running on different machines due to a large library of pre-requisite software, software versionning, or different OS and for modern, deep learning based algorithms, hardware is often a limiting factor due to requiring dedicated GPUs. 
// U-net source

#linebreak()
In this thesis work will be carried out to create an open source, user friendly and robust small blood vessel extraction method for obtaining segmented blood vessels, with the goal of enabling downstream extraction of clinically relevant vasculature characteristics. Analysis is done on contrast-enhanced µCT data acquired in the context of the testing of Pazopanib, with a focus on ensuring re-usability and adoption by researchers. The pipeline uses 3DSlicer as a basis, taking in and outputing data in portable data formats, to ensure re-usability and compatibility with existing software. Proven algorithms with user adjustable hyperparameters to enable easy re-use without requiring expertise in computer science are to be used. Implemented methods will be compared with existing pipelines for extracting fine blood vessels, and tested for robustness by examining performance on data previously considered non segmentable due to non uniform contrast agent staining.
