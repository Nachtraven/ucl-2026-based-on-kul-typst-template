...
#linebreak()
With the introduction of computing to the world of bio and medical imaging, higher levels of representation were also enabled, where the output of a pipeline may be the reconstruction of a structured output such as a network or skeleton, as is seen with software skeletonization methods @skan. Computers lend themselves well to information extraction and processing, especially compression. Different techniques or algorithms exist to extract information of interest from an image, ranging from simple classical methods such as the OTSU method for edge detection @OTSU_segmentation to more computationally expensive, complex and inscrutable deep learning. Deep learning algorithms are known to act as a form of information compression algorithm @ml_is_compression, starting from a data source with many individual data points, such as an image, and ending with as little as a single class output as illustrated in @alexnet_og_deeplearning.
...
As mentioned, machine learning relies on the process of iterative optimization, and can be as simple as optimizing for an intensity threshold. Deep learning methods, and in particular the paradigm of fully convolutional encoder-decoder architectures such as U-Net @unet_og_paper, represent a major revolution in the way image segmentation was done, and offered a leap in performance. U-Net presented breakthrough performance in bio and medical imaging due to its ability to segment large images with high compute and data efficiency, and across multiple scales by introducing skip connections between encoder and decoder pathways. These allow the network to combine low-level spatial detail with high-level semantic context, enabling the segmentation of thin structures that require context such as cells and, as relevant here, capillaries. However as mentioned previously, 2D imaging and by extension 2D segmentation present limitations for blood vessels; variants of U-Net extending into the third dimension exist to palliate this, such as 3D U-Net @3d_unet. 
