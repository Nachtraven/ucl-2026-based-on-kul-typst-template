...
// This work delivers the segmentation pipeline in the form of a plugin with downstream quantitative research analysis left to existing tools by exporting binary masks. Deep learning in its most common form is not utilized due to a lack of available reference data, training data and usability concerns. The plugin is intended for use in a research setting and tested on the provided data with its known limitations.
