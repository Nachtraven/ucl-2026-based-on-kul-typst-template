...
#figure(
  ...
  caption: [
    Representative central slices from the contrast-enhanced micro-CT data illustrating the principal challenges of automatic vasculature extraction in this regime. *(a)* A large high contrast vessel approximately 14 voxels in diameter. *(b)* A large high-intensity region not corresponding to vasculature, illustrating staining artefacts. *(c)* Upper, left: A bundle of small vessels in cross-section, ranging from 2 to 8 voxels in diameter, where the partial-volume effect is present, as well as compression artifacts. *(d)* Right hand side: the "shell effect", a high-intensity boundary surrounding the tumor caused by the diffusion of the contrast agent, which confounds global thresholding methods. *(e, f)* Two slices from the same volume, separated by 5 voxels along the z-axis. The vessel indicated appears discontinuous in (e) but continuous in (f), illustrating the utility of 3D methods.
  ],
) <cect-data-examples>