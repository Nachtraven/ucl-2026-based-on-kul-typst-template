...
#linebreak()
Therefore, in this thesis an open-source 3D Slicer plugin called CollaboratiVessel is proposed for microvasculature extraction from CECT data, constructed based on CECT datasets from murine tumors collected for the study of the antiangiogenic drug Pazopanib. Rather than requiring annotated data or computer science expertise, the end user is placed in the algorithmic loop by providing sparse input to an extraction pipeline that combines multi-scale vesselness filtering with ridge following and reconnection, ingesting and emitting standard formats to integrate with downstream analysis software. CollaboratiVessel is evaluated on CECT datasets with manually annotated ground truths, and is shown to recover vascular structure with improved connectedness than currently used intensity-based thresholding methods, enabling downstream analysis and supporting future research on vascular changes.
